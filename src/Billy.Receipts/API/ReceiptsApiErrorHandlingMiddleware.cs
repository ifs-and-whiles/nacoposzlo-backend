...
{
    public class ReceiptsApiErrorHandlingMiddleware
    {
        private readonly RequestDelegate _next;
        private static readonly ILogger Logger = Log.ForContext<ReceiptsApiErrorHandlingMiddleware>();
        
        public ReceiptsApiErrorHandlingMiddleware(RequestDelegate next)
        {
            _next = next;
        }
        ...
        {
            if (context.Request.Path.StartsWithSegments($"/{ReceiptsApiRouteConsts.Prefix}"))
            {
                try
                {
                    await _next(context);
                ...
                catch (SecurityException exception)
                {
                    ...
                    await HandleSecurityException(context, exception);
                }
                ...
                //Skip processing if request does to belong to Receipts API 
                await _next(context);
        } 
        ...
        private Task HandleSecurityException(HttpContext context, SecurityException securityException)
        {
            ...
            {
                StatusCode = context.Response.StatusCode,
                Message = securityException.Code
            }.ToString());
        }
